                ...
                <article class="post" data-post-id="{{ $post->id }}">
                    ...
                    <div class="panel-body">
                        ...
                        <div class="interaction">
                            | <a class="button is-small is-danger is-outlined" href="{{ route('post.show',  ['post_id' => $post->id]) }}"> Leer mas </a> 
                            {{-- <a href="#" class="like">{{ Auth::user()->likes()->where('post_id', $post->id)->first() ? Auth::user()->likes()->where('post_id', $post->id)->first()->like == 1 ? 'You like this post' : 'Like' : 'Like'  }}</a>
                            | --}}
                            {{-- <a href="#" class="like">{{ Auth::user()->likes()->where('post_id', $post->id)->first() ? Auth::user()->likes()->where('post_id', $post->id)->first()->like == 0 ? ' You don\'t like this post' : 'Dislike' : 'Dislike'  }}</a>|
                            --}}
                            @if(Auth::user() == $post->user)
                            <a href="#" class="button is-small is-danger is-outlined">Edit</a> 
                            <a href="{{ route('grafica',$post->id) }}" class="button is-small is-danger is-outlined">Stadisticas</a> 
                            <a class="button is-small is-danger is-outlined" href="{{ route('post.destroy', $post->id) }}">Delete</a>
                        </div>
                        @endif
